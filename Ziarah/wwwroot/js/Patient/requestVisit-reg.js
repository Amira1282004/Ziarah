...
// ══════════════════════════════════════════════════════════════════════════════
//  استقبال فلتر التخصص القادم من صفحة الرئيسية (?spec=...)
// ══════════════════════════════════════════════════════════════════════════════
function handleSpecFromUrl() {
    ...
    if (specSelect) {
        // ابحث عن الخيار الأقرب (مقارنة مُطبَّعة)
        ...
        var targetNorm = normalize(spec);
        var found = false;
        Array.from(specSelect.options).forEach(function (opt) {
            if (normalize(opt.value) === targetNorm || normalize(opt.text) === targetNorm) {
                specSelect.value = opt.value;
                found = true;
            }
        });
        // إذا لم نجد تطابقاً دقيقاً، نضع النص مباشرة (سيُطبَّق عبر filterName)
        if (!found) {
            var nameInput = document.getElementById("filterName");
            if (nameInput) nameInput.value = spec;
        }
    }

    updateDisplay();

    // إخفاء/إظهار فلتر التخصص
    var specFilter = document.getElementById("specFilter");
    if (specFilter) specFilter.style.display = "";

    // مرر لقائمة النتائج
    setTimeout(function () {
        var main = document.querySelector(".visit-main");
        if (main) window.scrollTo({ top: main.offsetTop - 80, behavior: "smooth" });
    }, 200);
}